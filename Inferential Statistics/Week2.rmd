...
set.seed(11121995)                 
```
The number above is completely arbitraty.You only need to do this once in your R Markdown document,
...
Calculating the mean and standard deviation of a sample in R is easy. And we know that the sample size is 60. So the only remaining building
...
z_star_95
```
which is roughly equal to the value critical value 1.96
...
x <- samp %>% summarise( lower = mean(area) - z_star_95 * (sd(area) / sqrt(n)),
              			 upper = mean(area) + z_star_95 * (sd(area) / sqrt(n))
                        )
ans <- ifelse(x$lower < params$mu & x$upper > params$mu, "yes", "no")
ans
```
...
<div id="exercise">
**Exercise**: Calculate 50 confidence intervals at the 99% confidence level. Plot all intervals and calculate the proportion of intervals that include the true population mean.
</div>
```{r plot-99-perc-cis}
ci_99 <- ames %>%
        rep_sample_n(size = n, reps = 50, replace = TRUE) %>%
...
plot_ci_99 <- ggplot(data = ci_99_data, aes(x = ci_bounds, y = ci_id,group = ci_id, color = capture_mu)) +
  geom_point(size = 2)+ 
  geom_line() + 
...
```{r}
count_99 = sum(ci_99$capture_mu == "yes")
prop_99 = count_99/50
prop_99
```
